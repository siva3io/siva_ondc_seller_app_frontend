...
export default function OmnichannelMain({ fields, updateFields }) {
  ...
  useEffect(
    () => dispatch(fetchAvailableMarketplace(fields?.leaf_category_id)),
    [fields?.leaf_category_id]
  );
  ...
  // setLocalData(localStorage.getItem("user_data"));
  const company_id = localData ? JSON.parse(localData)["company_id"] : 1;
      ...
      <Box sx={{ m: 1 }}>
        <TabContext value={value}>
            ...
            {enableTemplateMarketplaces ? (
              ...
              <>
                <div className="omnichannel-main-tab">
                  ...
                  >
                    {products &&
                      ...
                      products.map((product) => {
                        return (
                          <Tab label={product?.name} value={product?.id} />
                        );
                      })}
                  ...
                  <Button variant="outlined" onClick={handleRefreshMp}>
                    Refresh catalogue AI Values
                  </Button>
                </div>
              </>
            )}
          ...
          {products &&
            ...
            products.map((product) => {
              return (
                <TabPanel value={product.id}>
                  <Box>
                    <iframe
                      src={`https://dev-api.eunimart.com/webapp/catalogues?ext_category_id=${extCategory}&ext_channel_id=${product?.external_id}&variant_id=${fields?.id}&company_id=${company_id}`}
                      ref={catalog}
                      id="catalog"
                      title="catalog"
                      style={{
