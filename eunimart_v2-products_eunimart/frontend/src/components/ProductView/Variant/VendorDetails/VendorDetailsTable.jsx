...
export default function VendorDetailsTable({
  ...
  const handleVendorTable = pricelistData => {
    ...
    const tempVendorListArrGenerator = pricelistData.map((elem, index) => {
      return {
        vendorName: elem.vendors.name,
        product: elem.product_variant.product_name,
        vendorLeadTime: elem.supply_lead_time.lead_time,
        costPrice: elem.price,
        vendorCreditPeriod: elem.credit_period.credit_period,
        id: elem.id,
      };
    });
    const another = { ...data };
    another["price_list_details"] = tempVendorListArrGenerator;
    setVendor(another);
    setSaveEnable(true);
    console.log("came into here", data, pricelistData);
    // let tempVendorListArrGenerator = [];
    // if (data.price_list_details) {
    //   setPricelistPopulateData(pricelistData); //prepopulating purpose

    //   tempVendorListArrGenerator = pricelistData.map((elem, index) => {
    //     let boolVal = true;
    ...
    //   setSaveEnable(true);
    // } else {
    //   console.log(data, pricelist, "prevdetails");
    //   let prev = { ...data, pricelistData };
    //   setVendor(prev);
    //   setSaveEnable(true);
      ...
      <Paper sx={{ width: "100%", mb: 2 }}>
        <TableContainer>
          ...
          >
            ...
            />
            <TableBody>
              {console.log("data_in_table", data)}
                ...
                data?.map((row, index) => { */
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="left">
                        {row.vendorLeadTime ? row.vendorLeadTime : "--"}
                      </TableCell>

                      <TableCell align="left">{row.costPrice}</TableCell>
                      <TableCell align="left">
                        {row.vendorCreditPeriod ? row.vendorCreditPeriod : "--"}
                      </TableCell>
                    </TableRow>
                  );
                })}
