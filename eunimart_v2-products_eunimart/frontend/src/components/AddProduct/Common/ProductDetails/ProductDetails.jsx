...
function ProductDetails({
  ...
  const { hsncode } = useSelector((state) => state.data);
  ...
  const setCheckboxType = (label, prop, value) => {
    ...
    if (label === "product_procurement_treatment_ids") {
      let index = temp[3].sub.findIndex(function (field) {
        return field[0].toString() === prop.toString();
      });
      temp[3].sub[index][2] = value;
      let tempFinalArr = temp[3].sub.map((item) => {
        if (item[2] === true) {
          return { id: item[0] };
        }
      });
  ...
  //     });
  //   }
  //   temp[1].data.unshift({ label: "+ Add New Brand", value: "create" });
  //   setStaticFields(temp);
  // }, [brand]);
  ...
  useEffect(() => {
    ...
    if (procurementType && procurementType.data) {
      temp[3].sub = procurementType.data.map((item) => {
        if (
          edit &&
