...
function Location({
  ...
  console.log("statesListData", statesListData);
  ...
  // const [dummy, seyDummy] = useState(0);
  ...
  const [staticFields, setStaticFields] = useState([
    ...
    },
    {
      ...
      type: "input",
      required: true,
      errorMessage: "Address line 3 is required",
      key: "address_line3",
    },
  ]);
  ...
  const onSelectionChanges = (prop, valueId, valueLabel) => {
    ...
    if (valueId !== "create") {
      ...
      setFinalData({ ...finalData, location: { ...tempAttribute, [prop]: valueId } });
  ...
  const onInputChange = (prop, value) => {
    ...
    setFinalData({ ...finalData, location: { ...tempAttribute, [prop]: value } });
  };

  useEffect(() => {
    let temp = [...staticFields];
    console.log("currency", statesListData);
    if (statesListData) {
      temp[2].data = statesListData.map((item) => {
        return {
          label: item.name,
          value: item.id,
        };
      ...
      <div className="locationDetailForm">
        {brand && (
          <div className="staticFormCard">
            <div className="staticFormCardTitle">Location</div>
