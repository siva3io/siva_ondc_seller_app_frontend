...
export default function PackagingTable({
  ...
  const [PackagingMaterial, setPackagingMaterial] = React.useState("");
  ...
  const onSelectionChange = (type, value, index) => {
    console.log("onSelectionChange", value, index);
    if (type == "packagingMaterial") {
      setPackagingMaterial(value.label);
    }
    // const tempPackageArr = [...packageList];
      ...
      >
        <TableCell>
          <Autocomplete
            ...
            onChange={(e, value) =>
              onSelectionChange("packagingMaterial", value, tempCount)
            }
            sx={{ width: "100%" }}
            // value={elem.name}
