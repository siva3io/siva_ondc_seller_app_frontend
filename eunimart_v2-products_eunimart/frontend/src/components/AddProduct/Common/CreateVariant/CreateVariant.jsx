...
function CreateVariant({
  ...
  console.log("variantTableIS", variantTable);
  ...
  const onSelectionChanges = (prop, valueId, valueLabel, index) => {
    console.log("onSelectionChanges", prop, valueId, valueLabel, index);
  ...
  useEffect(() => {
    if (propertyArr && propertyArr.length > 0) {
      ...
      // fianelArr = fianelArr[0];
      setGenerate({ ...generate, attribute_values: fianelArr });
    }
    console.log("***working useeffect", propertyArr);
  }, [propertyArr]);
  ...
  useEffect(() => {
    ...
    if (edit && step2Data.attribute_key_values && check1 === false) {
      ...
      console.log("attribute_key_values_console", step2Data);
      const tempAttriArr = step2Data.attribute_key_values.map(
        (attribute, indexAttri) => {
          console.log("consoleeee", attribute, indexAttri, propertiesApi);
          ...
          let tempProp = propertiesApi.data?.map(property => {
            ...
            if (
              ...
              property.base_attribute_id === attribute.id
            ) {
              return { value: property.id, label: property.name };
            }
          });
          ...
          console.log("consoleeee2", tempProp, tempPropVal);

          tempPropArr[indexAttri].data = tempProp;
          tempPropArr[indexAttri].value = tempPropVal ? tempPropVal : [];
          console.log("changing here 1st", tempPropArr);
          setPropertyArr(tempPropArr);

          console.log("changing here", propertyArr, tempPropArr);
      ...
      setCheck1(true);
      console.log("*****changed_final_here", tempFinalArr, finalAttributes);
      setFinalAttributes(tempFinalArr);
      console.log(
        "***** after changed_final_here",
        tempFinalArr,
        finalAttributes
      );
...
const AttributeRow = ({
  ...
  return (
    <Box className="addVariantBlock">
      {console.log("inputs are", attribute, property)}
