...
function ProductViewMdm({ edit }) {
  ...
  const ratingCategoryData = useSelector(
    state => state.data.ratingCategoryData
  );
  const [ratingCategoryId, setratingCategoryId] = useState();
  useEffect(() => {
    ratingCategoryData.map(o => {
      o?.category_name == "ITEM" && setratingCategoryId(o.id);
    });
  }, [ratingCategoryData]);
  const access = useSelector(state => state.access.access);

  useEffect(() => {
    console.log("ratingcategoryId is", ratingCategoryId);
    ratingCategoryId !== undefined &&
      dispatch(loadFeedbackData(id, ratingCategoryId));
  }, [ratingCategoryId]);
  ...
  useEffect(() => {
    ...
    dispatch(loadRatingCategoryList());
  }, []);
  ...
  useEffect(() => {
    console.log("branddetails", productApiData);
    setBrand(
