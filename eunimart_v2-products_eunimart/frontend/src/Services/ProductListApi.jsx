...
export const AvailableMarketplaceconfig = (param) => {
  return {
    method: "get",
    url: `${base_URL}/integrations/ai_data_sync/valid_channel_list/${param}`,
    headers: {
      "Content-Type": "application/json",
      Authorization: `${GLOBAL_API_SOURCE.token}`,
    },
...
export const AccessManagement = (param) => {
  return {
    ...
    url: `${base_URL}/api/v1/template/${location.hostname === "localhost"
        ? 1
        : localStorage.getItem("access_template_id")
      }?filters=[["display_name","=","PRODUCTS"]]`,
...
export const searchConfig = (searchValue, type, ondc) => {
  console.log("ondcondc",ondc)
  let url="";

  if (ondc==true) {
    console.log("entered")
     url = `${base_URL}/api/v1/products/variant?filters=[["channel","ilike","ONDC"]]`;
    ...
    // }
     ...
     url = `${base_URL}/api/v1/products/variant`;
