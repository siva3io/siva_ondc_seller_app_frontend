...
const Login = ({ setSessionId }) => {
  ...
  const PostVerifyData = async (id, otp) => {
    // localStorage.setItem("test","inPostVerify")
    ...
    if (
      ...
      if (localStorage.getItem("newUser") === "true") {
        ...
        const checkBuyerData = JSON.parse(localStorage.getItem("user_data"));
        console.log(checkBuyerData, "local storage newuser");
        if (
          checkBuyerData &&
          checkBuyerData.user_types &&
          checkBuyerData.user_types[0] &&
          checkBuyerData.user_types[0].name == "Buyer"
        ) {
          console.log("local storage newuser if");
          navigate.push("/storeFront");
        } else {
          console.log("local storage newuser else");
          navigate.push("/home");
        }
  ...
  useEffect(() => {
    handlePlayVideo();
    let user_data_is_there = JSON.parse(localStorage.getItem("user_data"))
    if (localStorage.getItem("user_data") && JSON.parse(localStorage.getItem("user_data")) && JSON.parse(localStorage.getItem("user_data"))?.company_id != undefined && user_data_is_there?.user_types?.length > 0) {
      if (user_data_is_there?.user_types[0].name && user_data_is_there?.user_types[0].name.toUpperCase() != "BUYER") {
        navigate.push("/home")
      }
      else{
        navigate.push("/storeFront")
      ...
      <div className="login-wrapper">
        ...
        <div className="main_wrapper">
          <div className="loginDiv">
            ...
            {stepN && stepN == "userDoesntExist" && step1 && step1 != "getOtp" && (
              <Box>
                <Typography
                  fontFamily="Poppins"
                  fontSize="31px"
